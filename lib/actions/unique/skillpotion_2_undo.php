...
else
{
  ...
  if($target_pet['user'] != $user['user'] || $target_pet[$pet_field] == 'no' || $target_pet['dead'] != 'no' || $target_pet['zombie'] == 'yes' || $target_pet['location'] != 'home')
  {
?>
<p>The <?= $this_inventory['itemname'] ?> can become a <?= $used_item_name ?> by taking the power of <?= $power_description ?> from one of your pets!</p>
...
<?php
    ...
    {
      if($this_pet[$pet_field] == 'no')
        echo '   <option disabled>' . $this_pet['petname'] . ' (does not have this power)</option>';
...
<?php
  ...
  else
  {
    ...
    echo '<p>The ' . $this_inventory['itemname'] . ' has become a ' . $used_item_name . '!</p>',
         '<p><a href="/pet/profile.php?petid=' . $petid . '">' . $target_pet['petname'] . '</a> has lost the power of ' . $power_description . '.</p>';
